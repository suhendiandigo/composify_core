...
def test_rule_async_metadata():
    ...
    assert rule.output_type.inner_type == Result
    assert list(rule.dependencies)[0].typing.inner_type == Param
...
def test_dependency_qualifiers():
    @rule(metadata=(SolveSpecificity.Exact, "name"))
    def test_rule(param: Param) -> Result:
        return Result(param.value)

    _rule = as_rule(test_rule)
    for d in _rule.dependencies:
        assert d.typing.solve_parameter.specificity == SolveSpecificity.Exact
        assert d.typing.solve_parameter.specificity == SolveSpecificity.Exact
